...
def streaming(images_batch,audio,streaming_server_url):
    ...
    process=(
        ...
        .output(
            vcodec='libx264', # [libx264,rawvideo,h264_nvenc]只有rawvideo才能随便设置ts长度，其他压缩格式各自有自己的最小长度，比如h264_nvenc最小10秒
            # preset="fast", # h264_nvenc.fast代表了一系列参数组合
            format='segment', 
            pix_fmt='rgb24', 
            segment_list=streaming_server_url+"/Playlist.m3u8", # 创建播放列表
            ...
            filename=streaming_server_url+"/stream_%03d.ts" # 创建ts片段
            )
        .overwrite_output() 
